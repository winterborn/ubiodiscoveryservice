...
describe("InstanceService", () => {
    let mesh;
    let instanceService;
    let instanceRepositoryMock;
    let metricsMock;
    beforeEach(() => {
        mesh = new Mesh();
        instanceRepositoryMock = sinon.createStubInstance(InstanceRepository);
        metricsMock = sinon.createStubInstance(Metrics);
        metricsMock.heartbeatRate = { incr: sinon.stub() };
        metricsMock.errorRate = { incr: sinon.stub() };
        metricsMock.expiredInstances = { incr: sinon.stub() };
        metricsMock.activeInstances = { set: sinon.stub() };
    ...
    it("should record a heartbeat and return the instance", async () => {
        ...
        const result = await instanceService.recordHeartbeat("test-group", "instance-1", { key: "value" });
    ...
    it("should return false when removing a non-existent instance with suppressErrors set to true", async () => {
        instanceRepositoryMock.deleteInstance.resolves(false);
        const result = await instanceService.removeInstance("non-existent-group", "non-existent-id", true);
        expect(result).to.be.false;
    });
});
//# sourceMappingURL=InstanceService.test.js.map